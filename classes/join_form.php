...
class SELECTGENDER_CLASS_JoinForm extends JoinForm
{
    ...
    public function getQuestions()
    {
        ...
        $config = OW::getConfig();
        $opositeMatchSexJoin = $config->getValue('selectgender', 'opositeMatchSexJoin');
        $sameMatchSexJoin = $config->getValue('selectgender', 'sameMatchSexJoin');


        $jsParams = array(
            'opositeMatchSexJoin' => $opositeMatchSexJoin,
            'sameMatchSexJoin' => $sameMatchSexJoin
        );

        $script = ' var join = new SELECTGENDER_Join(); join.init('.  json_encode($jsParams).'); ';

        OW::getDocument()->addOnloadScript($script);
        OW::getDocument()->addScript(OW::getPluginManager()->getPlugin('selectgender')->getStaticJsUrl().'join.js');
